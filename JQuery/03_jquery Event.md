...
**$.holdReady() <jQuery.holdReady()>** : DOM이 로드되는 것을 지연시켜준다. ; 
js 파일을 불러들이고 동작을 할때, js가 로드되지 않은 시점에 DOM이 로드 되는것을 방지해준다.
 / head 영역에 선언하는게 좋다.
/*DOM =>DOM은 문서 객체 모델(Document Object Model)의 줄임말입니다. 
이는 HTML, XML 또는 다른 문서 유형의 구조화된 표현을 생성하고, 
이를 자바스크립트와 같은 프로그래밍 언어를 사용하여 조작할 수 있게 해주는 표준화된 방법
웹 페이지의 DOM은 요소(element), 속성(attribute), 텍스트 등의 
노드(node)로 이루어진 트리 구조로 표현됩니다. 
자바스크립트는 이 DOM 구조를 사용하여 웹 페이지의 콘텐츠를 동적으로 변경하고 
조작할 수 있습니다.

`$.holdReady()` 또는 `jQuery.holdReady()`는 jQuery의 메서드로, 
이 메서드를 사용하면 자바스크립트 파일이나 코드가 로드되는 것을 지연시킬 수 있습니다. 
이는 일반적으로 DOM이 로드된 이후에 특정한 동작이 실행되어야 할 때 유용합니다. 
이 메서드를 사용하면 자바스크립트 파일이 로드되지 않은 상태에서 
DOM 요소에 접근하거나 조작하는 것을 방지할 수 있습니다.
이 메서드는 특히 웹 페이지의 `<head>` 영역에서 선언하는 것이 좋습니다. 
이렇게 함으로써 자바스크립트 파일이나 코드가 로드되기 전에 
DOM이 완전히 로드되도록 보장할 수 있습니다.*/
...
**.error()** : 선택된 개체(images, window, document 등)가 정확하게 로드(load)되지 않을 경우 
발생 오류 이벤트
...
**event.which** : 이벤트가 발생된 요소의 key (키보드 값<숫자>, 
마우스 <왼쪽 1, 중간 2, 오른쪽 3>)
...
**event.stopImmediatePropagation()** : 다른 이벤트 핸들러가 호출되는 것을 막는다. 
(이벤트 버블링)

**event.isPropagationStopped()** : 
event.stopPropagation()이 호출 됐는지 여부 리턴 true / false

**event.isImmediatePropagationStopped()** : 
event.stopImmediatePropagation()이 호출됐는지 여부 리턴 true /false****
```
